...
public class CrearUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        UsuariosJpaController sjc = new UsuariosJpaController(emf);
        Usuarios s = new Usuarios();
        s.setNombre("Alvaro");
        s.setContrasena("2026AlAl.");
        s.setCorreo("alvaroaradilla@gmail.com");
        s.setRol(TipoUsuario.CLIENTE);
        ParcelaJpaController pjc = new ParcelaJpaController(emf);
        List<Parcela> p = pjc.findParcelaEntities();
        s.setParcelas(p);
        sjc.create(s);
        emf.close();
